...
- `src/lib/dom-to-png.ts` powers the PNG export on `/templates` and `/brand`: it
  clones the card as authored, inlines the page's own stylesheets and the
  self-hosted fonts, and rasterises that through an SVG `<foreignObject>`, so
  the download matches the on-screen card instead of a computed-style copy of
  it. State persists in `localStorage` under `vg-tpl-state-v1`.
- Brand webfonts are self-hosted in `public/fonts/` with `src/styles/fonts.css`
  generated by `npm run gen:fonts` — the export can only inline fonts it can
  read same-origin, and it keeps a third-party request off every page.
